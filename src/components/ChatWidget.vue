...
const props = defineProps({
  festival: { type: Object, default: () => ({ name: '', description: '' }) }
})

const open = ref(false)
const input = ref('')
const messages = ref([
  { role: 'assistant', content: `안녕하십니꺼! ${props.festival.name}에 대해 물어보이소.` }
])
...
const systemIntro = computed(() =>
  `당신은 부산 축제 안내 챗봇이야. 말을 부산 사투리로 해야 해. 존댓말은 부드럽게 쓰고, "그라믄", "아이구", "캡짱", "카이", "머라카노" 같은 표현을 자연스럽게 섞어서 답해라. 축제: ${props.festival.name || ''} - ${props.festival.description || ''}`
)
